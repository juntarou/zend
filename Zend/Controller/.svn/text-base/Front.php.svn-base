...
class Zend_Controller_Front
{
    ...
    public function dispatch(Zend_Controller_Request_Abstract $request = null, Zend_Controller_Response_Abstract $response = null)
    {
		...
		if ($this->_response->getException()) {
		    $this->setLoggerException($this->_response->getException());
		}
    ...
    public function setLoggerException($ex)
    {
	//ob_start(array($this,"obException"));
	$message = "";
	...
	foreach ($ex as $e) {
		...
		foreach ($e->getTrace() as $key => $t) {
		    ...
		    if ($count = count($t['args'])) {
			foreach ($t['args'] as $key => $a) {
			    if (is_object($a)) {
				$message .= "(" . get_class($a) . ")" . PHP_EOL;
				break;
			    }
			    if ($key === 0) {
				if ($key === ($count - 1)) {
	...
	$this->logPutException($message);
	//ob_end_clean();
    }

    public function logPutException($buffer)
    {
	// year directory (20xx/)
	$directory = LOG_DIR . date("Y") . DIRECTORY_SEPARATOR;

	// check year directory exests
	if (!is_dir($directory)) {
	...
	// log file
	$fileName = $directory . date("d") . EXT_TEXT;

	// log file exests
	if (!file_exists($fileName)) {

	    file_put_contents($fileName, date("Y/m/d") . "LOGS" . PHP_EOL);
	    chmod($fileName,0777);

	}

	file_put_contents($fileName, PHP_EOL.$buffer,FILE_APPEND);
    }
}
